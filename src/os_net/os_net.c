...
static void packet_handler(uint8_t *args, const struct pcap_pkthdr *header, const uint8_t *packet)
{
    // Parse the packet here and extract the information you need
    printf("Packet captured: Length %d\n", header->len);
    // Add your logic to extract and store the required fields
}
        ...
        if (os_net_agt)
        {
            if (os_net_agt->lip)
            {
                if (bind(ossock, local_ai->ai_addr, local_ai->ai_addrlen))
                {
                    ...
                    verbose("Connecting from local address %s", os_net_agt->lip);
            }
        }
